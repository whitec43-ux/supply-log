...
export default function App() {
  ...
  const [pinPrompt,     setPinPrompt]     = useState(null); // stores sub waiting for PIN
  ...
  // Archive (replaces true delete)
  const deleteSub = async (id, table) => {
    try {
      const records = table === "submissions" ? supplySubs : dailySubs;
      const record = records.find(s => s.id === id);
      if (record) {
        await supabase.from("archives").insert({ id: `${id}-${Date.now()}`, source_table: table, data: record });
      }
  ...
  // PIN check for daily check deletions
  const handleDeletePress = (sub, table) => {
    if (table === "daily_checks") {
      setPinValue("");
      setPinError(false);
      setPinPrompt({ ...sub, _table: table });
    } else {
      setConfirmDelete({ ...sub, _table: table });
    }
  };
  ...
  const PinModal = () => !pinPrompt ? null : (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 24 }}>
      <div style={{ background: "#fff", borderRadius: 16, padding: 24, maxWidth: 340, width: "100%", boxShadow: "0 8px 32px rgba(0,0,0,0.2)" }}>
        ...
        <h3 style={{ fontSize: 17, fontWeight: 700, textAlign: "center", margin: "0 0 8px", color: "#0f172a" }}>Enter PIN to delete</h3>
        <p style={{ fontSize: 13, color: "#94a3b8", textAlign: "center", margin: "0 0 20px" }}>Daily Check entries are PIN-protected</p>
        <input
          type="password" inputMode="numeric" pattern="[0-9]*" maxLength={6}
          value={pinValue} onChange={e => { setPinValue(e.target.value); setPinError(false); }}
          onKeyDown={e => e.key === "Enter" && submitPin()}
          placeholder="······"
          style={{ width: "100%", padding: "12px 14px", borderRadius: 10, border: `1.5px solid ${pinError ? "#dc2626" : "#e2e8f0"}`, fontSize: 24, fontWeight: 700, textAlign: "center", letterSpacing: "0.3em", outline: "none", boxSizing: "border-box", marginBottom: 8 }}
          autoFocus
        />
        {pinError && <p style={{ color: "#dc2626", fontSize: 13, textAlign: "center", margin: "0 0 12px", fontWeight: 600 }}>Incorrect PIN. Try again.</p>}
        <div style={{ display: "flex", gap: 10, marginTop: pinError ? 0 : 12 }}>
          <button onClick={() => { setPinPrompt(null); setPinValue(""); setPinError(false); }}
            style={{ ...C.ghostBtn, flex: 1, padding: "11px 0", borderRadius: 10, textAlign: "center" }}>Cancel</button>
          <button onClick={submitPin}
            style={{ flex: 1, padding: "11px 0", borderRadius: 10, border: "none", background: "#0891b2", color: "#fff", fontWeight: 600, fontSize: 14, cursor: "pointer" }}>
            Continue
          </button>
        </div>
      </div>
      ...
      <div style={C.page}>
        <div style={C.card}>
          ...
          <div style={C.row}>
            ...
            <Stepper value={dcScope} onChange={setDCScope} max={9} />
          </div>
                ...
                <div key={sub.id} style={{ ...C.setupCard, marginBottom: 12 }}>
                  ...
                  <div style={{ padding: "10px 16px" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "flex-end", paddingTop: 6 }}>
                      <button onClick={() => handleDeletePress(sub, "daily_checks")}
                ...
                <div key={setupName} style={C.setupCard}>
                  ...
                  <div style={{ padding: "12px 16px" }}>
                    ...
                    {ss.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp)).map(sub => {
                      ...
                      const subAutoItems = AUTO_ITEMS.filter(i => sub.autoQtys?.[i.id] > 0);
                      const subManualItems = MANUAL_ITEMS.filter(i => sub.manualQtys?.[i.id] > 0);
                      const subRareLines = RARE_ITEMS.flatMap(item => {
                        const r = sub.rare?.[item.id];
                        if (!r?.checked) return [];
                        if (item.sizes) return item.sizes.filter(sz => r.sizeQtys?.[sz] > 0).map(sz => `${item.label} ${sz} ×${r.sizeQtys[sz]}`);
                        return r.qty > 0 ? [`${item.label} ×${r.qty}`] : [item.label];
                      });
                      const subEmergencyLines = EMERGENCY_MEDS.filter(m => sub.emergencyMeds?.[m.id]).map(m => m.label);
                      const subLowItems = LOW_ITEMS.filter(i => sub.lowChecked?.[i.id]);
                      return (
                        <div key={sub.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                          <div style={{ display: "flex", alignItems: "flex-start", padding: "8px 0", cursor: "pointer" }}
                            ...
                            <div style={{ flex: 1 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                ...
                                <span style={{ fontSize: 11, color: "#94a3b8" }}>
                                  submitted {new Date(sub.timestamp).toLocaleDateString("en-US", { month: "short", day: "numeric" })} at {fmtTime(sub.timestamp)}
                                </span>
                              </div>
                              <div style={{ fontSize: 12, color: "#94a3b8", marginTop: 1 }}>
                                {sub.patients} patients · {sub.initials || "—"}
                                {sub.comments && <span style={{ fontStyle: "italic" }}> · "{sub.comments}"</span>}
                              </div>
                            </div>
                            <div style={{ display: "flex", alignItems: "center", gap: 4, flexShrink: 0 }}>
                              <span style={{ fontSize: 11, color: "#94a3b8" }}>{isExpanded ? "▲" : "▼"}</span>
                              <button onClick={e => { e.stopPropagation(); handleDeletePress(sub, "submissions"); }}
                                style={{ background: "none", border: "none", cursor: "pointer", fontSize: 16, color: "#cbd5e1", padding: "2px 4px" }}>🗑</button>
                            </div>
                          </div>
                          {isExpanded && (
                            <div style={{ background: "#f8fafc", borderRadius: 10, padding: "12px 14px", marginBottom: 10 }}>
                              {(subAutoItems.length > 0 || subManualItems.length > 0) && <>
                                <div style={C.reviewTitle}>Items Used</div>
                                {subAutoItems.map(i => (
                                  <div key={i.id} style={C.reviewRow}><span>{i.label}</span><span style={C.blueBadge}>{sub.autoQtys[i.id]}</span></div>
                                ))}
                                {subManualItems.map(i => (
                                  <div key={i.id} style={C.reviewRow}><span>{i.label}</span><span style={C.blueBadge}>{sub.manualQtys[i.id]}</span></div>
                                ))}
                              </>}
                              {(subRareLines.length > 0 || subEmergencyLines.length > 0) && <>
                                <div style={{ ...C.reviewTitle, marginTop: 10 }}>Rare / Emergency Items</div>
                                {subRareLines.map((l, i) => <div key={i} style={C.reviewRow}><span style={{ fontSize: 13 }}>{l}</span></div>)}
                                {subEmergencyLines.map((l, i) => <div key={"e"+i} style={C.reviewRow}><span style={{ fontSize: 13, color: "#dc2626" }}>{l}</span></div>)}
                              </>}
                              {subLowItems.length > 0 && <>
                                <div style={{ ...C.reviewTitle, marginTop: 10, color: "#dc2626" }}>Needs Restocking</div>
                                {subLowItems.map(i => (
                                  <div key={i.id} style={C.reviewRow}><span>{i.label}</span><span style={C.redBadge}>Low</span></div>
                                ))}
                              </>}
                              {subAutoItems.length === 0 && subManualItems.length === 0 && subRareLines.length === 0 && subEmergencyLines.length === 0 && subLowItems.length === 0 && (
                                <p style={{ fontSize: 13, color: "#94a3b8", margin: 0 }}>No items recorded.</p>
                              )}
                            </div>
                          )}
                        </div>
