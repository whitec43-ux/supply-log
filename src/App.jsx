...
export default function App() {
  ...
  const [submissionDate, setSubmissionDate] = useState(() => fmtDate(new Date()));
  ...
  const [confirmDelete, setConfirmDelete] = useState(null); // stores submission to delete
  ...
  const submit = async () => {
    const entry = {
      ...
      timestamp: new Date(submissionDate + "T12:00:00").toISOString(),
      date: submissionDate,
  ...
  const reset = () => {
    setStep(0); setSetup(""); setPatients(0); setInitials(""); setSubmissionDate(fmtDate(new Date()));
  ...
  const exportOne = (setupName) => {
    ...
    const setupSubs = subs.filter(s => s.setup === setupName && new Date(s.timestamp) >= weekStart && new Date(s.timestamp) < end);
      ...
      <div style={C.page}>
        <div style={C.card}>
          <div style={C.topBar}>
            ...
            <button onClick={reset} style={C.ghostBtn}>← Back</button>
          </div>
              ...
              return (
                <div key={setupName} style={C.setupCard}>
                  ...
                  <div style={{ padding: "12px 16px" }}>
                    ...
                    {/* Individual submissions with delete */}
                    ...
                    {ss.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp)).map(sub => (
                      <div key={sub.id} style={{ ...C.adminRow, alignItems: "center" }}>
                        <div>
                          <span style={{ fontSize: 13, fontWeight: 500 }}>
                            {new Date(sub.timestamp).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })}
                          </span>
                          <span style={{ fontSize: 12, color: "#94a3b8", marginLeft: 8 }}>{sub.patients} patients · {sub.initials || "—"}</span>
                        </div>
                        <button
                          onClick={() => setConfirmDelete(sub)}
                          style={{ background: "none", border: "none", cursor: "pointer", fontSize: 16, color: "#cbd5e1", padding: "2px 4px" }}
          ...
          {confirmDelete && (
            <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 24 }}>
              <div style={{ background: "#fff", borderRadius: 16, padding: 24, maxWidth: 340, width: "100%", boxShadow: "0 8px 32px rgba(0,0,0,0.2)" }}>
                ...
                <h3 style={{ fontSize: 17, fontWeight: 700, textAlign: "center", margin: "0 0 8px", color: "#0f172a" }}>Delete this submission?</h3>
                <p style={{ fontSize: 14, color: "#64748b", textAlign: "center", margin: "0 0 6px" }}>
                  {confirmDelete.setup} · {new Date(confirmDelete.timestamp).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })}
                </p>
                <p style={{ fontSize: 13, color: "#94a3b8", textAlign: "center", margin: "0 0 24px" }}>
                  {confirmDelete.patients} patients · {confirmDelete.initials || "—"}
                </p>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  ...
                  <button onClick={() => deleteSub(confirmDelete.id)} style={{ flex: 1, padding: "11px 0", borderRadius: 10, border: "none", background: "#dc2626", color: "#fff", fontWeight: 600, fontSize: 14, cursor: "pointer" }}>Yes, Delete</button>
                </div>
              </div>
            </div>
          )}
        ...
        {step === 4 && <>
          ...
          <div style={C.reviewBlock}>
            ...
            <div style={{ ...C.reviewRow, color: "#94a3b8", fontSize: 13 }}>
              <span>{new Date(submissionDate + "T12:00:00").toLocaleDateString("en-US", { weekday: "long", month: "short", day: "numeric" })}</span>
            </div>
          </div>
