...
const AUTO_ITEMS = [
  { id: "pedsCircuit",   label: "Peds Circuit",                        tag: "n",   calc: n => n },
  ...
  { id: "syr5",          label: "5cc Syringes (tubes + routine meds)", tag: "n+4", calc: n => n + 4 },
];
...
export default function App() {
  ...
  const exportCSV = () => {
    ...
    const headers = ["Date", "Submitted By", "Setup", "Patients",
      ...AUTO_ITEMS.map(i => i.label),
      ...MANUAL_ITEMS.map(i => i.label),
      "Rare Items",
      ...LOW_ITEMS.map(i => `LOW: ${i.label}`)
    ];

    const rows = filtered.map(s => {
      const rareText = RARE_ITEMS.flatMap(item => {
      ...
      return [s.date, s.initials || "—", s.setup, s.patients,
        ...AUTO_ITEMS.map(i => s.autoQtys?.[i.id] ?? 0),
        ...MANUAL_ITEMS.map(i => s.manualQtys?.[i.id] ?? 0),
        rareText,
        ...LOW_ITEMS.map(i => s.lowChecked?.[i.id] ? "YES" : "")
      ];
    });
    ...
    const a = Object.assign(document.createElement("a"), {
      ...
      download: `supply-log-${fmtDate(weekStart)}.csv`
    });
    a.click();
  };
